...
public class PillAlarm implements Comparable<PillAlarm> {
    ...
    public String getPillPhotoPath() {return pillPhotoPath;}

    public void setPillPhotoPath(String pillPhotoPath) {this.pillPhotoPath = pillPhotoPath;}
    /**
     * Overrides the compareTo() method so that alarms can be sorted by time of day from earliest to
     * latest.
     */
    @Override
    public int compareTo(PillAlarm anotherPillAlarm) {
        if (hour < anotherPillAlarm.getHour())
            return -1;
        else if (hour > anotherPillAlarm.getHour())
            return 1;
        else {
            if (minute < anotherPillAlarm.getMinute())
                return -1;
            else if (minute > anotherPillAlarm.getMinute())
                return 1;
            else
                return 0;
        }
